...
interface Content {
  // ... other properties ...
  genre: string | string[]; // Allow genre to be either a string or string array
  // ... other properties ...
}
...
export default function ContentGrid({ type, filter }: ContentGridProps) {
  ...
  const documentaryItems = filteredItems.filter(
    (item) => item.type === "documentary"
  );
  const podcastItems = filteredItems.filter(
