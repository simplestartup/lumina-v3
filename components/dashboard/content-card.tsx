...
export default function ContentCard({ content }: ContentCardProps) {
  ...
  const handleWatchedToggle = () => {
    ...
    toast.success(
      content.watched ? "Marked as unwatched" : "Marked as watched"
    );
  };
  ...
  const handleRating = (value: number) => {
    ...
    toast.success(
      content.rating === value ? "Rating removed" : `Rated ${value} stars`
    );
  };
  ...
  const handleSave = () => {
    const genreToSave = Array.isArray(editedContent.genre)
      ? editedContent.genre.join(", ")
      : editedContent.genre;

    updateContent(content.id, { ...editedContent, genre: genreToSave });
        ...
        <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Are you sure you want to remove "{content.title}" from your
    ...
    <Card className="group relative h-full flex flex-col overflow-hidden">
      ...
      <CardContent className="flex-1 p-4">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {(Array.isArray(content.genre) ? content.genre : [content.genre]).map(
            (genre) => (
              <Badge key={genre} variant="outline" className="text-xs">
                {genre}
              </Badge>
